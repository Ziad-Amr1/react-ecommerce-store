    ...
    return (
        <div className="mt-6 flex items-center justify-center gap-4">
            <Button variant="outline" className="cursor-pointer border-border bg-background text-foreground hover:bg-muted"
                disabled={currentPage === 1 || isFetching} onClick={() => setCurrentPage((prev) => prev - 1)}>{t("products.previous")}
            </Button>
            ...
            <Button variant="outline" className="cursor-pointer border-border bg-background text-foreground hover:bg-muted"
                disabled={currentPage === totalPages || isFetching} onClick={() => setCurrentPage((prev) => prev + 1)}>{t("products.previous")}
            </Button>
        </div>
    );
}