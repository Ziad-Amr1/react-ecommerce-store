    ...
    return (
        <div className="rounded-xl border border-border bg-card p-6 shadow-sm lg:col-span-1">
            ...
            <div className="space-y-4">
                ...
                <p className="font-mono text-xs text-muted-foreground">{images.length}/5 images</p>
                ...
                {images.length > 0 && ( 
                    <div className="grid grid-cols-2 gap-3"> 
                        {images.map((file, index) => ( 
                            <div key={`${file.name}-${index}`} className="relative overflow-hidden rounded-lg border border-border bg-background shadow-sm"> 
                                <ProductImagePreview file={file} alt={`Product ${index + 1}`} /> 
