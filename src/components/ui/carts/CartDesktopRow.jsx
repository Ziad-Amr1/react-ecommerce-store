...
export function CartDesktopRow({ cart, isExpanded, onToggle, onOpenDetails }) {
  const total = cartTotal(cart.items);
  const itemCount = cartItemCount(cart.items);

  // <tr> isn't focusable or keyboard-activatable by default, so we make it
  // act like a button: focusable via tabIndex, and Enter/Space trigger it.
  ...
  return (
    <>
      ...
      >
        <td className="px-4 py-3.5">
          <div className="flex items-center gap-3">
            ...
            <div className="leading-tight">
              <div className="text-sm font-medium">{cart.customer}</div>
              <div className="text-xs text-muted-foreground">{cart.email}</div>
            </div>
          </div>
        </td>

        <td className="px-4 py-3.5">
          <span className="font-mono text-xs text-muted-foreground">{cart.id}</span>
        </td>
        ...
        <td className="px-4 py-3.5 text-sm text-muted-foreground">{cart.updatedAt}</td>
