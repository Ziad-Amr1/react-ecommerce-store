...
export default function About() {
  ...
  const values = [
    {
      ...
      description: t("about.values.quality.description", {
        defaultValue:
          "Every item in our store undergoes rigorous inspection to ensure lasting satisfaction.",
      }),
    },
    {
      id: "shipping",
      icon: Truck,
      title: t("about.values.shipping.title", {
        defaultValue: "Fast & Reliable Shipping",
      }),
      description: t("about.values.shipping.description", {
        defaultValue:
          "We partner with top global logistics carriers to get your orders to your doorstep quickly.",
      }),
    },
    {
      id: "secure",
      ...
      title: t("about.values.secure.title", {
        defaultValue: "100% Secure Checkout",
      }),
      description: t("about.values.secure.description", {
        defaultValue:
          "Encrypted transactions and safe payment processing give you peace of mind.",
      }),
    },
    {
      id: "support",
      ...
      description: t("about.values.support.description", {
        defaultValue:
          "Our friendly customer care team is available around the clock to assist you.",
      }),
    },
  ];
  ...
  const stats = [
    {
      value: 12500,
      label: t("about.stats.customers", { defaultValue: "Happy Customers" }),
      suffix: "+",
    },
    {
      value: 850,
      label: t("about.stats.products", { defaultValue: "Curated Products" }),
      suffix: "+",
    },
    {
      value: 99.8,
    ...
    {
      value: 24,
      label: t("about.stats.support", { defaultValue: "Global Support" }),
      suffix: "/7",
    },
  ];
        ...
        <div>
          <div className="mx-auto max-w-3xl text-center space-y-4">
            <div className="inline-flex items-center gap-2 rounded-full border border-(--color-border) bg-(--color-surface) px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-(--color-text-secondary) shadow-xs">
              <Sparkles
                className="size-3.5 text-(--color-primary)"
                aria-hidden="true"
              />
            ...
            <h1 className="font-display text-3xl font-bold tracking-tight text-(--color-text-primary) sm:text-5xl">
              {t("about.heroTitle", {
                defaultValue: "Redefining Online Shopping",
              })}
            </h1>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="text-center space-y-2 mb-12">
            <h2 className="font-display text-2xl font-bold tracking-tight sm:text-3xl text-(--color-text-primary)">
              {t("about.valuesTitle", {
                defaultValue: "Why Choose Oversea Store",
              })}
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {values.map((item) => {
              ...
              return (
                <Card
                  key={item.id}
                  className="border-(--color-border) bg-(--color-surface) transition-shadow hover:shadow-md"
                >
          ...
          <div className="grid grid-cols-1 gap-12 lg:grid-cols-2 lg:items-center">
            <div className="space-y-6">
              ...
              <p className="text-base leading-relaxed text-(--color-text-secondary)">
                {t("about.missionDescription", {
                  defaultValue:
                    "At Oversea Store, we believe that online shopping should be seamless, inspiring, and dependable. Our team hand-picks every product in our catalog to ensure it meets strict quality standards.",
                })}
              </p>
              ...
              <ul className="space-y-3">
                {[
                  t("about.bullet1", {
                    defaultValue: "Rigorous quality check on every vendor item",
                  }),
                  t("about.bullet2", {
                    defaultValue: "Transparent pricing with zero hidden fees",
                  }),
                  t("about.bullet3", {
                    defaultValue:
                      "Fast global shipping with full parcel tracking",
                  }),
                ].map((bullet, idx) => (
                  <li
                    key={idx}
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              {stats.map((stat, idx) => (
                <div
                  key={idx}
                  className="flex flex-col justify-center rounded-2xl border border-(--color-border) bg-(--color-surface-secondary) p-4 sm:p-6 text-center"
                >
                  <p className="font-display text-2xl font-bold tabular-nums text-(--color-text-primary) sm:text-3xl lg:text-4xl">
                    {formatNumber(stat.value, locale)}
                    {stat.suffix}
                  </p>
                  <p className="mt-1 text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-(--color-text-secondary)">
                    {stat.label}
                  </p>
                </div>
              ))}
      ...
      <section className="py-16 sm:py-20 text-center">
        <div className="mx-auto max-w-4xl space-y-6">
          <h2 className="font-display text-2xl font-bold tracking-tight text-(--color-text-primary) sm:text-4xl">
            {t("about.ctaTitle", {
              defaultValue: "Ready to Experience Better Shopping?",
            })}
          </h2>
          ...
          <p className="text-base text-(--color-text-secondary) max-w-xl mx-auto">
            {t("about.ctaSubtitle", {
              defaultValue:
                "Browse our latest arrivals and exclusive collections today.",
            })}
          </p>
          ...
          <Button
            asChild
            size="lg"
            className="rounded-full gap-2 px-8 cursor-pointer"
          >
            <Link to="/products">
