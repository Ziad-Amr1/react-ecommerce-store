import { Eye, Pencil, Trash2, PackageOpen, MoreHorizontal, ArrowUpDown, ArrowDown, ArrowUp } from "lucide-react";
    ...
    <div className="overflow-hidden rounded-lg border bg-card shadow-sm">
      <TooltipProvider delayDuration={0}>
        <Table density="compact">
          <TableHeader>
            <TableRow>
              <TableHead className="text-start">{t("products.columns.product")}</TableHead>
              <TableHead className="text-start">{t("products.columns.category")}</TableHead>
              <TableHead className="text-start">{t("products.columns.brand")}</TableHead>
              <TableHead className="text-start">
                <button
                  type="button"
                  onClick={onPriceSort}
                  disabled={isFetching}
                  className="inline-flex cursor-pointer items-center gap-1 font-medium hover:text-primary disabled:cursor-not-allowed disabled:opacity-50"
                >
              ...
              <TableHead className="text-start">{t("products.columns.stock")}</TableHead>
              <TableHead className="text-end">{t("products.columns.actions")}</TableHead>
            </TableRow>
          </TableHeader>
          ...
          <TableBody>
            {products.length === 0 ? (
                ...
                <TableCell colSpan={6}>
                  <div className="flex h-52 flex-col items-center justify-center gap-3">
                    ...
                    <div className="text-center">
                      <p className="font-medium text-foreground">{t("products.noProductsFound")}</p>
                      <p className="mt-1 text-sm text-muted-foreground">{t("products.noProductsHint")}</p>
                    </div>

                    {hasActiveQuery && (
                      <Button variant="outline" className="cursor-pointer" onClick={onClearQuery}>
              ...
              products.map((product) => {
                ...
                return (
                  <TableRow key={product._id} className="group cursor-pointer" onClick={() => onProductClick(product._id)}>
                    <TableCell>
                      <div className="flex max-w-72 items-center gap-3">
                        <ProductThumb url={product.images?.[0]?.url} alt={product.name} />
                        <div className="min-w-0">
                          <TruncateWithTooltip value={product.name} className="max-w-40 font-medium text-foreground" />
                          {product.tags?.[0] && (
                            <p className="truncate text-xs text-muted-foreground">{product.tags[0]}</p>
                          )}
                        </div>
                      </div>
                    </TableCell>
                    ...
                    <TableCell className="text-muted-foreground">
                      <span className="block max-w-36 truncate">{product.category || "—"}</span>
                    </TableCell>

                    <TableCell className="text-muted-foreground">
                      <span className="block max-w-36 truncate">{product.brand || "—"}</span>
                    </TableCell>
                      ...
                      {product.price == null ? (
                        ...
                        <div className="flex items-center gap-2">
                          <span className="font-medium text-foreground">
                            {formatCurrency(product.discountPrice, CURRENCY, i18n.language)}
                          </span>
                    ...
                    <TableCell className="whitespace-nowrap">
                      <span className={`tabular-nums font-medium ${stockClass(product.stock)}`}>
                        {product.stock == null ? "—" : product.stock}
                      </span>
                    </TableCell>

                    <TableCell className="whitespace-nowrap text-end" onClick={(event) => event.stopPropagation()}>
