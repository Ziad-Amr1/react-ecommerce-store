...
export default function AddProductGallery({ images, errors, isSubmitting, onImageChange, onRemoveImage }) {
  ...
  return (
    <Card className="self-start lg:col-span-1">
      ...
      <CardContent className="space-y-4">
        <label className={`flex min-h-45 flex-col items-center justify-center rounded-lg border border-dashed border-border transition ${isFull || isSubmitting ? "cursor-not-allowed opacity-50" : "cursor-pointer hover:bg-muted"}`}>
          ...
          <span className="mt-1 text-xs text-muted-foreground">{t("products.uploadImagesHint")}</span>
        ...
        {images.length > 0 && (
          <div className="grid grid-cols-2 gap-3">
            {images.map((file, index) => (
              <div key={`${file.name}-${index}`} className="relative aspect-square overflow-hidden rounded-lg border bg-background shadow-sm">
                <ProductImagePreview file={file} alt={t("products.imagePreviewAlt", { index: index + 1 })} />
